...
def _connect_and_wait(client, host=BROKER_HOST, port=BROKER_PORT, timeout=5.0):
    """Connect client (with loop_start), poll until CONNACK received. Returns True on success."""
    connected = [False]
    orig = client.on_connect

    def _cb(c, userdata, flags, reason_code, props=None):
        if reason_code == 0:
            connected[0] = True
        if orig:
            orig(c, userdata, flags, reason_code, props)
...
class TestRetainAsPublished:
    ...
    def test_rap_false_clears_retain_flag(self):
        ...
        clear_client.username_pw_set(USERNAME, PASSWORD)
        assert _connect_and_wait(clear_client), "clear_client did not connect"
        ...
        assert _connect_and_wait(pub_client), "pub_client did not connect"
    ...
    def test_rap_non_retained_message(self):
        ...
        sub_client_false.username_pw_set(USERNAME, PASSWORD)
        ...
        sub_client_true.username_pw_set(USERNAME, PASSWORD)
        ...
        time.sleep(0.5)  # Allow broker to fully register subscriptions
    ...
    def test_rap_multiple_subscribers_different_settings(self):
        ...
        sub1.username_pw_set(USERNAME, PASSWORD)
        ...
        sub2.username_pw_set(USERNAME, PASSWORD)
        ...
        sub3.username_pw_set(USERNAME, PASSWORD)
