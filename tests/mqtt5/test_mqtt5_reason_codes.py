...
- Run: python3 tests/test_mqtt5_reason_codes.py
...
def test_mqtt5_reason_codes():
    ...
    print("=" * 70)
    print("MQTT v5.0 Reason Codes Test")
    print("=" * 70)
    ...
    client_id = f"test_mqtt5_rc_{int(time.time())}"
    ...
    try:
        ...
        test_topics = [
            ("test/reason_codes/topic1", 1),
            ("test/reason_codes/topic2", 2),
        ]
        ...
        # Wait for SUBACK
        start = time.time()
        while not state["suback_received"] and (time.time() - start) < 5.0:
            time.sleep(0.1)
        ...
        publish_topic = "test/reason_codes/publish"
        ...
        # Wait for PUBACK
        start = time.time()
        while state["puback_reason_code"] is None and (time.time() - start) < 5.0:
            time.sleep(0.1)
        ...
        # Wait for UNSUBACK
        start = time.time()
        while not state["unsuback_received"] and (time.time() - start) < 5.0:
            time.sleep(0.1)
...
if __name__ == "__main__":
    print("\nStarting MQTT v5.0 Reason Codes test...")
