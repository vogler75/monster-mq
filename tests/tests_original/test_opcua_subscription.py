...
class OPCUASubscriber:
    ...
    def datachange_notification(self, node, val, data):
        logger.info(f"📡 OPC UA subscription received: node={node}, value={val}, timestamp={data.server_timestamp}")
        self.received_notifications.append({
            'node': str(node),
            'value': val,
            'timestamp': data.server_timestamp.timestamp() if data.server_timestamp else time.time()
        })
...
async def test_opcua_subscription_notifications():
    ...
    try:
        async with Client(url=url) as client:
            ...
            subscription = await client.create_subscription(500, OPCUASubscriber())
            ...
            subscription.subscription_handler.received_notifications.clear()
            ...
            # Check results
            opcua_notifications = subscription.subscription_handler.received_notifications
