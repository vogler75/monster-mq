...
def main():
    ...
    for name, port in backends:
        results[name] = run_backend(name, port)
...
def test_all_backends():
    """Pytest entry point: run Phase 5 tests across all backends."""
    exit_code = main()
    assert exit_code == 0, "One or more backends failed Phase 5 tests"
