...
class OpcUaExtension : AbstractVerticle() {
    ...
    private fun setupEventBusHandlers(): Future<Void> {
        ...
        try {
            ...
            vertx.eventBus().consumer<MqttMessage>(ADDRESS_OPCUA_VALUE_PUBLISH) { message ->
    ...
    private fun handleOpcUaValuePublish(message: Message<MqttMessage>) {
        try {
            ...
            when (operation) {
                "add", "update", "addAddress", "deleteAddress" -> {
                    ...
                    if (device.isAssignedToNode(currentNodeId) && device.enabled) {
                        // Redeploy connector for this device to pick up address changes
