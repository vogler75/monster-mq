...
/**
 * Neo4jExtension
 * Coordinates deployment of Neo4jConnector instances for devices of type DEVICE_TYPE_NEO4J_CLIENT.
 */
class Neo4jExtension : AbstractVerticle() {
    ...
    companion object {
        const val ADDRESS_DEVICE_CONFIG_CHANGED = "neo4j.device.config.changed"
    }

    override fun start(startPromise: Promise<Void>) {
        try {
            currentNodeId = Monster.getClusterNodeId(vertx)
            deviceRegistry = vertx.sharedData().getLocalMap("neo4j.device.registry")
    ...
    private fun loadAndDeployDevices(): Future<Void> {
        ...
        deviceStore.getEnabledDevicesByNode(currentNodeId).onComplete { res ->
            if (res.succeeded()) {
                val devices = res.result().filter { it.type == DeviceConfig.DEVICE_TYPE_NEO4J_CLIENT }
                ...
                devices.forEach { dev ->
                    deployConnector(dev).onComplete { dRes ->
                        processed++
                        if (!dRes.succeeded()) {
                            logger.warning("Failed to deploy Neo4jConnector for ${dev.name}: ${dRes.cause()?.message}")
                        }
    ...
    private fun deployConnector(device: DeviceConfig): Future<String> {
        ...
        if (!device.isAssignedToNode(currentNodeId)) {
            p.fail(Exception("Device not assigned to this node"))
            return p.future()
        }
        val cfg = JsonObject().put("device", device.toJsonObject())
        val options = DeploymentOptions().setConfig(cfg)
        vertx.deployVerticle(Neo4jConnector(), options).onComplete { res ->
            if (res.succeeded()) {
                deployedConnectors[device.name] = res.result()
                activeDevices[device.name] = device
                deviceRegistry[device.namespace] = device.name
                logger.info("Deployed Neo4jConnector for ${device.name}: ${res.result()}")
                p.complete(res.result())
            } else {
                logger.severe("Failed to deploy Neo4jConnector for ${device.name}: ${res.cause()?.message}")
                p.fail(res.cause())
            }
        }
        return p.future()
    ...
    private fun undeployConnector(deviceName: String): Future<Void> {
        ...
        logger.info("Undeploying Neo4jConnector for $deviceName")
    ...
    private fun setupEventBusHandlers(): Future<Void> {
        ...
        try {
            ...
            // List connectors
            vertx.eventBus().consumer<JsonObject>(EventBusAddresses.Neo4jBridge.CONNECTORS_LIST) { msg ->
                try {
        ...
        try {
            ...
            when (op) {
                "add", "update", "addAddress", "deleteAddress" -> {
                    ...
                    if (device.type != DeviceConfig.DEVICE_TYPE_NEO4J_CLIENT) {
