 ...
 * NatsClientExtension
 *
 * Coordinator verticle that manages deployments of [NatsClientConnector] instances
 * for all devices of type [DeviceConfig.DEVICE_TYPE_NATS_CLIENT] assigned to the
 * current cluster node.
 */
class NatsClientExtension : AbstractVerticle() {
    ...
    companion object {
        const val ADDRESS_DEVICE_CONFIG_CHANGED = "natsclient.device.config.changed"
    }

    override fun start(startPromise: Promise<Void>) {
        try {
            currentNodeId = Monster.getClusterNodeId(vertx)
            deviceRegistry = vertx.sharedData().getLocalMap("natsclient.device.registry")
    ...
    private fun loadAndDeployDevices(): Future<Void> {
        ...
        deviceStore.getEnabledDevicesByNode(currentNodeId).onComplete { res ->
            if (res.succeeded()) {
                val devices = res.result().filter { it.type == DeviceConfig.DEVICE_TYPE_NATS_CLIENT }
                ...
                devices.forEach { dev ->
                    deployConnector(dev).onComplete { dRes ->
                        processed++
                        if (!dRes.succeeded())
                            logger.warning("Failed to deploy NatsClientConnector for ${dev.name}: ${dRes.cause()?.message}")
    ...
    private fun deployConnector(device: DeviceConfig): Future<String> {
        ...
        vertx.deployVerticle(NatsClientConnector(), DeploymentOptions().setConfig(cfg)).onComplete { res ->
            if (res.succeeded()) {
    ...
    private fun setupEventBusHandlers(): Future<Void> {
        ...
        try {
            ...
            vertx.eventBus().consumer<JsonObject>(EventBusAddresses.NatsBridge.connectorsList(currentNodeId)) { msg ->
                try {
        ...
        try {
            ...
            when (op) {
                "add", "update", "addAddress", "updateAddress", "deleteAddress" -> {
                    ...
                    if (device.type != DeviceConfig.DEVICE_TYPE_NATS_CLIENT) {
