...
/**
 * SparkplugB Decoder Extension - Main coordination verticle for SparkplugB decoder device management
 *
 * Responsibilities:
 * - Cluster-aware device management (only manages devices assigned to current node)
 * - Deploys/undeploys SparkplugBDecoderConnector verticles per device
 * - Handles configuration changes via EventBus
 * - Routes MQTT messages to appropriate connectors
 */
class SparkplugBDecoderExtension : AbstractVerticle() {
    ...
    companion object {
        // EventBus addresses
        const val ADDRESS_DEVICE_CONFIG_CHANGED = "sparkplugb.decoder.device.config.changed"
        const val ADDRESS_SPARKPLUGB_DECODER_PUBLISH = "sparkplugb.decoder.value.publish"
    }
    ...
    override fun start(startPromise: Promise<Void>) {
        ...
        try {
            ...
            logger.severe("Exception during SparkplugBDecoderExtension startup: ${e.message}")
            startPromise.fail(e)
        }
    }
    ...
    override fun stop(stopPromise: Promise<Void>) {
        ...
        Future.all<Void>(undeployFutures as List<Future<Void>>)
            ...
            .onComplete { result ->
                if (result.succeeded()) {
                    logger.info("SparkplugBDecoderExtension stopped successfully")
                    stopPromise.complete()
                } else {
                    logger.warning("Error during SparkplugBDecoderExtension shutdown: ${result.cause()?.message}")
    ...
    private fun initializeDeviceStore(): Future<Void> {
        ...
        try {
            ...
            logger.warning("DeviceConfigStore not implemented for this store type, SparkplugB Decoder features will be disabled")
    ...
    private fun loadAndDeployDevices(): Future<Void> {
        ...
        deviceStore.getEnabledDevicesByNode(currentNodeId)
            .onComplete { result ->
                if (result.succeeded()) {
                    // Filter to only include SparkplugB Decoder devices
                        ...
                        deployConnectorForDevice(device)
                            .onComplete { deployResult ->
                                ...
                                if (completedCount == devices.size) {
                                    logger.info("SparkplugB Decoder deployment completed: $successCount/$completedCount devices deployed successfully")
                                    promise.complete()
                                }
                            }
                    }
    ...
    private fun deployConnectorForDevice(device: DeviceConfig): Future<String> {
        ...
        try {
            ...
            vertx.deployVerticle(SparkplugBDecoderConnector(), options)
                .onComplete { result ->
                    if (result.succeeded()) {
    ...
    private fun setupEventBusHandlers(): Future<Void> {
        ...
        try {
            ...
            // Handle SparkplugB decoder value publishing to MQTT bus
            vertx.eventBus().consumer<BrokerMessage>(ADDRESS_SPARKPLUGB_DECODER_PUBLISH) { message ->
                handleSparkplugBDecoderValuePublish(message)
            }

            // Provide list of active connector device names - node-specific address
            vertx.eventBus().consumer<JsonObject>(EventBusAddresses.SparkplugBDecoder.connectorsList(currentNodeId)) { msg ->
                try {
    ...
    private fun handleSparkplugBDecoderValuePublish(message: Message<BrokerMessage>) {
        try {
            val mqttMessage = message.body()
            logger.fine { "Forwarding SparkplugB decoded value to MQTT bus: ${mqttMessage.topicName} = ${String(mqttMessage.payload)}" }
            ...
            if (sessionHandler != null) {
                ...
                logger.severe("SessionHandler not available for SparkplugB decoded message publishing")
            }

        } catch (e: Exception) {
            logger.severe("Error forwarding SparkplugB decoded value to MQTT bus: ${e.message}")
        }
    }
