...
/**
 * WinCC Unified Extension - Main coordination verticle for WinCC Unified device management
 *
 * Responsibilities:
 * - Cluster-aware device management (only manages devices assigned to current node)
 * - Deploys/undeploys WinCCUaConnector verticles per device
 * - Handles configuration changes via EventBus
 * - Routes MQTT subscriptions to appropriate connectors
 */
class WinCCUaExtension : AbstractVerticle() {
    ...
    companion object {
        // EventBus addresses
        const val ADDRESS_DEVICE_CONFIG_CHANGED = "winccua.device.config.changed"
        const val ADDRESS_WINCCUA_VALUE_PUBLISH = "winccua.value.publish"
    }
    ...
    override fun start(startPromise: Promise<Void>) {
        ...
        try {
            ...
            logger.severe("Exception during WinCCUaExtension startup: ${e.message}")
            startPromise.fail(e)
        }
    }
    ...
    private fun initializeDeviceStore(): Future<Void> {
        ...
        try {
            ...
            logger.warning("DeviceConfigStore not implemented for this store type, WinCC UA features will be disabled")
    ...
    private fun loadAndDeployDevices(): Future<Void> {
        ...
        deviceStore.getEnabledDevicesByNode(currentNodeId)
            .onComplete { result ->
                if (result.succeeded()) {
                    // Filter to only include WinCC UA Client devices
                        ...
                        deployConnectorForDevice(device)
                            .onComplete { deployResult ->
                                ...
                                if (completedCount == devices.size) {
                                    logger.info("WinCC UA device deployment completed: $successCount/$completedCount devices deployed successfully")
                                    promise.complete()
                                }
                            }
                    }
        ...
        if (deploymentId != null) {
            vertx.undeploy(deploymentId)
                .onComplete { result ->
                    ...
                    if (result.succeeded()) {
                        logger.info("Undeployed WinCCUaConnector for device $deviceName")
                        promise.complete()
                    } else {
    ...
    private fun setupEventBusHandlers(): Future<Void> {
        ...
        try {
            ...
            // Handle WinCC UA value publishing to MQTT bus
            vertx.eventBus().consumer<BrokerMessage>(ADDRESS_WINCCUA_VALUE_PUBLISH) { message ->
                handleWinCCUaValuePublish(message)
            }

            // Provide list of active connector device names - node-specific address
            vertx.eventBus().consumer<JsonObject>(EventBusAddresses.WinCCUaBridge.connectorsList(currentNodeId)) { msg ->
                try {
    ...
    private fun handleWinCCUaValuePublish(message: Message<BrokerMessage>) {
        try {
            val mqttMessage = message.body()
            logger.fine { "Forwarding WinCC UA value to MQTT bus: ${mqttMessage.topicName} = ${String(mqttMessage.payload)}" }
            ...
            if (sessionHandler != null) {
                ...
                logger.severe("SessionHandler not available for WinCC UA message publishing")
            }

        } catch (e: Exception) {
            logger.severe("Error forwarding WinCC UA value to MQTT bus: ${e.message}")
        }
    }
            ...
            when (operation) {
                "add", "update", "addAddress", "deleteAddress" -> {
                    ...
                    if (device.isAssignedToNode(currentNodeId) && device.enabled) {
                        ...
                        logger.info("Deploying WinCC UA connector for device $deviceName on node $currentNodeId")
