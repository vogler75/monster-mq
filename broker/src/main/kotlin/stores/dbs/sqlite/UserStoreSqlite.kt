...
class UserStoreSqlite(
    ...
    private fun toBool(value: Any?, default: Boolean = false): Boolean {
        return when (value) {
            is Boolean -> value
    ...
    private fun mapRowToUser(row: JsonObject): User {
        return User(
            username = row.getString("username"),
            passwordHash = row.getString("password_hash"),
            enabled = toBool(row.getValue("enabled"), true),
            canSubscribe = toBool(row.getValue("can_subscribe"), true),
            canPublish = toBool(row.getValue("can_publish"), true),
            isAdmin = toBool(row.getValue("is_admin"), false),
            createdAt = row.getString("created_at")?.let { java.time.LocalDateTime.parse(it.replace(" ", "T")) },
            updatedAt = row.getString("updated_at")?.let { java.time.LocalDateTime.parse(it.replace(" ", "T")) }
        )
    }

    private fun mapRowToAclRule(row: JsonObject): AclRule {
        return AclRule(
            id = row.getValue("id").toString(),
            username = row.getString("username"),
            topicPattern = row.getString("topic_pattern"),
            canSubscribe = toBool(row.getValue("can_subscribe"), false),
            canPublish = toBool(row.getValue("can_publish"), false),
    ...
    override fun getUser(username: String): Future<User?> {
        ...
        return sqliteClient.executeQuery(sql, params).map { results ->
            if (results.size() > 0) {
                mapRowToUser(results.getJsonObject(0))
            } else null
        }.otherwise { e ->
    ...
    override fun getAllUsers(): Future<List<User>> {
        ...
        return sqliteClient.executeQuery(sql).map { results ->
            results.map { mapRowToUser(it as JsonObject) }
        }.otherwise { e ->
    ...
    override fun getAclRule(id: String): Future<AclRule?> {
        ...
        return sqliteClient.executeQuery(sql, params).map { results ->
            if (results.size() > 0) {
                mapRowToAclRule(results.getJsonObject(0))
            } else null
        }.otherwise { e ->
    ...
    override fun getUserAclRules(username: String): Future<List<AclRule>> {
        ...
        return sqliteClient.executeQuery(sql, params).map { results ->
            results.map { mapRowToAclRule(it as JsonObject) }
        }.otherwise { e ->
