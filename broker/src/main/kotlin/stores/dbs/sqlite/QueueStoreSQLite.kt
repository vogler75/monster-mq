 ...
 */
class QueueStoreSQLite(
    private val dbPath: String,
    private val visibilityTimeoutSeconds: Int = 30,
    private val storeType: QueueStoreType = QueueStoreType.SQLITE
    ...
    override fun start(startPromise: Promise<Void>) {
        ...
        sqlClient.initDatabase(createTableSQL).onComplete { result ->
            if (result.succeeded()) {
                logger.info("SQLite queue table '$tableName' is ready")
                startPromise.complete()
            } else {
                logger.severe("Failed to initialize SQLite queue table: ${result.cause()?.message}")
    ...
    // -------------------------------------------------------------------------
    // Purge: no-ops because ACK deletes rows and there are no orphan payload rows.
    // -------------------------------------------------------------------------
