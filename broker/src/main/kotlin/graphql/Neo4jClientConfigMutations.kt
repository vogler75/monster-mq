    ...
    fun createNeo4jClient(): DataFetcher<CompletableFuture<Map<String, Any>>> {
        return DataFetcher { env ->
            ...
            try {
                ...
                if (!Monster.getEnabledFeaturesForNode(request.nodeId).contains("Neo4j"))
                    return@DataFetcher future.apply { complete(mapOf("success" to false, "errors" to listOf("Neo4j feature is not enabled on node ${request.nodeId}"))) }
    ...
    fun reassignNeo4jClient(): DataFetcher<CompletableFuture<Map<String, Any>>> {
        return DataFetcher { env ->
            ...
            try {
                ...
                if (!Monster.getEnabledFeaturesForNode(nodeId).contains("Neo4j"))
                    return@DataFetcher future.apply { complete(mapOf("success" to false, "errors" to listOf("Neo4j feature is not enabled on node $nodeId"))) }
