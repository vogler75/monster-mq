...
/**
 * InfluxDBLoggerExtension
 * Coordinates deployment of InfluxDB Logger instances.
 */
class InfluxDBLoggerExtension : AbstractVerticle() {
    ...
    companion object {
        const val ADDRESS_DEVICE_CONFIG_CHANGED = "influxdb.logger.device.config.changed"
    }

    override fun start(startPromise: Promise<Void>) {
        try {
            currentNodeId = Monster.getClusterNodeId(vertx)
            deviceRegistry = vertx.sharedData().getLocalMap("influxdb.logger.device.registry")
            ...
            initializeDeviceStore()
                ...
                .onComplete { res ->
                    if (res.succeeded()) {
                        logger.fine("InfluxDBLoggerExtension started successfully")
                        startPromise.complete()
                    } else {
                        logger.severe("Failed to start InfluxDBLoggerExtension: ${res.cause()?.message}")
                        startPromise.fail(res.cause())
                    }
                }
        } catch (e: Exception) {
            logger.severe("Error starting InfluxDBLoggerExtension: ${e.message}")
            startPromise.fail(e)
        }
    }
        ...
        try {
            ...
            if (store != null) {
                ...
                deviceStore.initialize().onComplete { initRes ->
                    if (initRes.succeeded()) p.complete() else p.fail("Failed to init device store")
                }
    ...
    private fun loadAndDeployDevices(): Future<Void> {
        ...
        deviceStore.getEnabledDevicesByNode(currentNodeId).onComplete { res ->
            if (res.succeeded()) {
                val devices = res.result().filter { it.type == DeviceConfig.DEVICE_TYPE_INFLUXDB_LOGGER }
                if (devices.isEmpty()) {
                    p.complete()
                    return@onComplete
                }
    ...
    private fun setupEventBusHandlers(): Future<Void> {
        ...
        vertx.eventBus().consumer<JsonObject>(EventBusAddresses.InfluxDBLoggerBridge.CONNECTORS_LIST) { msg ->
        ...
        try {
            ...
            when (op) {
                "add", "update" -> {
                    ...
                    if (device.type != DeviceConfig.DEVICE_TYPE_INFLUXDB_LOGGER) {
                    ...
                    if (device.isAssignedToNode(currentNodeId) && device.enabled) {
                        ...
                        undeployLogger(name).onComplete { message.reply(JsonObject().put("success", true)) }
                    }
                }
