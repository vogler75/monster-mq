...
class Plc4xClientDetailManager {
    ...
    renderAddresses() {
        ...
        this.clientData.config.addresses.forEach(address => {
            ...
            const publishFormat = address.publishFormat || 'JSON';
            const publishFormatTitle = publishFormat === 'VALUE' ? 'Publish only the transformed value' : 'Publish JSON envelope';
            ...
            row.innerHTML = `
                ...
                <td>
                    ...
                    <small class="qos-value" title="${publishFormatTitle}">${publishFormat}</small>
                </td>
                <td>
    ...
    async addAddress() {
        ...
        const addressData = {
            ...
            publishFormat: document.getElementById('address-publish-format').value
        };
    ...
    async deleteAddress(addressName) {
        this.deleteAddressName = addressName;
        if (await ui.confirmDelete(addressName, { title: 'Delete address mapping' })) {
    ...
    showAddAddressModal() {
        ...
        document.getElementById('address-publish-format').value = 'JSON';
    ...
    editAddress(addressName) {
        ...
        document.getElementById('address-publish-format').value = address.publishFormat || 'JSON';
    ...
    async showDeleteModal() {
        const name = this.clientData ? this.clientData.name || this.clientName : 'this item';
        if (await ui.confirmDelete(name, { title: 'Delete PLC4X client' })) {
            this.deleteClient();
        }
    }
