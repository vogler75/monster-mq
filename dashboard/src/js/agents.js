...
class AgentManager {
    ...
    async loadAvailableFilters() {
        try {
            ...
            (result.agents || []).forEach(agent => {
                if (agent.org) orgSet.add(agent.org);
                if (!this.selectedOrg || agent.org === this.selectedOrg) {
                    if (agent.site) siteSet.add(agent.site);
                }
            ...
            this.availableOrgs = Array.from(orgSet).sort((a, b) => a.localeCompare(b));
            this.availableSites = Array.from(siteSet).sort((a, b) => a.localeCompare(b));
            ...
            console.warn('Could not load agent filters:', error.message);
            this.availableTags = [];
            this.availableOrgs = [];
            this.availableSites = [];
            this.renderNamespaceFilters();
    ...
    async loadAgents() {
        ...
        try {
            const query = `
                query GetAgents($org: String, $site: String, $tags: [String!], $tagFilterMode: String) {
                    agents(org: $org, site: $site, tags: $tags, tagFilterMode: $tagFilterMode) {
                        name
                        description
                        tags
                        org
    ...
    renderAgentsTable() {
        ...
        this.agents.forEach(agent => {
            ...
            const tagsHtml = this.renderAgentTags(agent.tags || []);
            ...
            row.innerHTML = `
                ...
                <td>${tagsHtml}</td>
                <td>${this.escapeHtml((agent.org || '') + ' / ' + (agent.site || ''))}</td>
                <td>${providerModel || '-'}</td>
                <td>
    ...
    renderAgentTags(tags) {
        ...
        return `<div class="agent-tags">${tags.map(tag => `<span class="agent-tag" style="${this.tagStyle(tag)}">${this.escapeHtml(tag)}</span>`).join('')}</div>`;
    }
    ...
    renderTagFilters() {
        ...
        container.innerHTML = tags.map(tag => {
            ...
            return `
                <button type="button" class="tag-chip${selected}" style="${this.tagStyle(tag)}" data-tag="${this.escapeHtml(tag)}" aria-pressed="${pressed}">
                    ${this.escapeHtml(tag)}
                </button>
            `;
        }).join('');
        ...
        container.querySelectorAll('.tag-chip').forEach(chip => {
            chip.addEventListener('click', () => {
                this.onTagFilterChange(chip.dataset.tag, chip.getAttribute('aria-pressed') !== 'true');
            });
        });
    }
    ...
    renderSelectOptions(id, values, selectedValue) {
        ...
        select.innerHTML = '<option value="">All</option>' + values
            .map(value => `<option value="${this.escapeHtml(value)}" ${value === selectedValue ? 'selected' : ''}>${this.escapeHtml(value)}</option>`)
            .join('');
    }
    ...
    async showEditModal(agentName) {
        ...
        try {
            ...
            document.getElementById('agent-allowedPublishTopics').value = (agent.allowedPublishTopics || []).join(', ');
    ...
    resetForm() {
        ...
        document.getElementById('agent-allowedPublishTopics').value = '';
    ...
    buildInputFields(data, isUpdate) {
        ...
        input.allowedPublishTopics = data.allowedPublishTopics;
    ...
    async deleteAgent(agentName) {
        ...
        if (await ui.confirmDelete(agentName, { title: 'Delete AI agent' })) {
