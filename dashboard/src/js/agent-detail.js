...
class AgentDetailManager {
    ...
    async showNewAgentForm() {
        ...
        document.getElementById('agent-allowed-publish-topics').value = '';
        ...
        document.getElementById('agent-visible-tags').value = '';
        document.getElementById('agent-isolated').checked = false;
    ...
    renderAgentInfo() {
        ...
        document.getElementById('agent-tags').value = (d.tags || []).join(', ');
        ...
        document.getElementById('agent-allowed-publish-topics').value = (d.allowedPublishTopics || []).join('\n');
        ...
        document.getElementById('agent-visible-tags').value = (d.visibleAgentTags || []).join(', ');
        document.getElementById('agent-isolated').checked = d.isolatedAgent || false;
    ...
    collectFormData() {
        ...
        const outputTopics = document.getElementById('agent-output-topics').value
            .split('\n').map(t => t.trim()).filter(t => t.length > 0);
        const allowedPublishTopics = document.getElementById('agent-allowed-publish-topics').value
            .split('\n').map(t => t.trim()).filter(t => t.length > 0);
        ...
        const data = {
            ...
            tags: this.parseCommaSeparatedList(document.getElementById('agent-tags').value),
            ...
            visibleAgentTags: this.parseCommaSeparatedList(document.getElementById('agent-visible-tags').value),
            isolatedAgent: document.getElementById('agent-isolated').checked
        };
    ...
    async showDeleteModal() {
        const name = this.agentData ? this.agentData.name : 'this item';
        if (await ui.confirmDelete(name, { title: 'Delete AI agent' })) {
            this.deleteAgent();
        }
    }
...
function setupContextDropZones() {
    ...
    const allowedPublishTopicsTextarea = document.getElementById('agent-allowed-publish-topics');
    ...
    [retainedTextarea, inputTopicsTextarea, outputTopicsTextarea, allowedPublishTopicsTextarea].forEach(textarea => {
        if (!textarea) return;
