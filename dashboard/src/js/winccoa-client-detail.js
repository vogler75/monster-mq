...
class WinCCOaClientDetailManager {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <ix-icon-button icon="pen" variant="subtle-tertiary" size="16" title="Edit Query" onclick="clientDetailManager.editQuery(${index})"></ix-icon-button>
                        <ix-icon-button icon="trashcan" variant="subtle-tertiary" size="16" class="btn-delete" title="Delete Query" onclick="clientDetailManager.deleteQuery(${index})"></ix-icon-button>
                    </div>
                </td>
            `;
    ...
    async showDeleteModal() {
        ...
        if (await ui.confirmDelete(name, { title: 'Delete WinCC OA client' })) {
            this.deleteClient();
        }
    }
