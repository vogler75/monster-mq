    ...
    class LoginManager {
        ...
        async checkBrokerConfigApiAvailable() {
            ...
            try {
                ...
                var brokers = Array.isArray(payload && payload.brokers) ? payload.brokers : payload;
        ...
        showGuestAccess() {
            if (this.guestAccess) this.guestAccess.style.display = 'flex';
            this.updateSecondaryActionsLayout();
        }
        ...
        showBrokerConfigAccess() {
            if (this.brokerConfigAccess) this.brokerConfigAccess.style.display = 'flex';
            this.updateSecondaryActionsLayout();
        }

        hideBrokerConfigAccess() {
            if (this.brokerConfigAccess) this.brokerConfigAccess.style.display = 'none';
            this.updateSecondaryActionsLayout();
        }

        updateSecondaryActionsLayout() {
            if (!this.secondaryActions) return;

            var actions = Array.prototype.slice.call(this.secondaryActions.querySelectorAll('.secondary-action'));
            var visibleActions = actions.filter(function(action) {
                return action.style.display !== 'none';
            });

            this.secondaryActions.style.display = visibleActions.length ? 'grid' : 'none';
            this.secondaryActions.classList.toggle('single-action', visibleActions.length === 1);
        }
