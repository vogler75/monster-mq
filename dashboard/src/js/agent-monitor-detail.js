...
class AgentDetailMonitorManager {
    ...
    renderLogs() {
        ...
        container.innerHTML = entries.map(entry => {
            ...
            const text = this.stringifyPayload(entry.payload);
            return `<div class="log-entry"><span class="log-ts">${this.esc(ts)}</span><pre class="log-payload">${this.esc(text)}</pre></div>`;
        }).join('');
