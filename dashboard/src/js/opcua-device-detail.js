...
class OpcUaDeviceDetailManager {
    ...
    async deleteAddress(address) {
        ...
        if (await ui.confirmDelete(address, { title: 'Delete address mapping' })) {
    ...
    async showDeleteModal() {
        const name = this.device ? this.device.name || this.deviceName : 'this item';
        if (await ui.confirmDelete(name, { title: 'Delete OPC UA client' })) {
            this.deleteClient();
        }
    }
