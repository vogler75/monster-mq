class ArchiveGroupsManager {
    ...
    async loadEnabledFeatures() {
        try {
            ...
            console.warn('Failed to fetch enabled features for archive groups:', e);
            this.redisServerEnabled = false;
        }
    }
            ...
            <tr>
                ...
                <td>
                    <div class="action-buttons">
                        ${group.enabled ?
                    `<ix-icon-button icon="pause" variant="subtle-tertiary" size="24" title="Disable" onclick="archiveGroupsManager.toggleArchiveGroup('${this.escapeHtml(group.name)}', false)"></ix-icon-button>` :
                    `<ix-icon-button icon="play" variant="subtle-tertiary" size="24" title="Enable" onclick="archiveGroupsManager.toggleArchiveGroup('${this.escapeHtml(group.name)}', true)"></ix-icon-button>`
                }
                        <a href="/pages/archive-group-detail.html?name=${encodeURIComponent(group.name)}"><ix-icon-button icon="highlight" variant="subtle-tertiary" size="24" title="Edit"></ix-icon-button></a>
                        <ix-icon-button icon="trashcan" variant="subtle-tertiary" size="24" class="btn-delete" title="${group.enabled ? 'Disable the archive group first to delete' : 'Delete'}"
        ...
        try {
            ...
            const groups = (result.archiveGroups || []).map(g => {
                const obj = {
                    ...
                    DatabaseConnectionName: g.databaseConnectionName || null,
                    PayloadFormat: g.payloadFormat
                };
                if (this.redisServerEnabled && g.redisDbNumber !== null && g.redisDbNumber !== undefined) obj.RedisDbNumber = g.redisDbNumber;
...
if (window.graphqlClient) {
    window.graphqlClient.getArchiveGroups = async function () {
        ...
        const redisDbNumberField = Array.isArray(features) && features.includes('RedisServer') ? 'redisDbNumber' : '';
        const query = `
            query GetArchiveGroups {
                archiveGroups {
                    name
