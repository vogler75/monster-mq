...
class WinCCUaClientDetailManager {
    ...
    async loadClient() {
        ...
        try {
            const query = `
                query GetWinCCUaClients($name: String!) {
                    winCCUaClients(name: $name) {
                        name
                        namespace
                        nodeId
                        enabled
            ...
            this.clientData = result.winCCUaClients && result.winCCUaClients.length > 0 ? result.winCCUaClients[0] : null;
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <ix-icon-button icon="edit" variant="primary" ghost size="16" title="Edit Subscription" onclick="clientDetailManager.editAddress(${index})"></ix-icon-button>
    ...
    async createClient(clientData) {
        ...
        if (result.winCCUaDevice.create.success) {
            ...
            setTimeout(() => {
                window.spaLocation.href = '/pages/winccua-client-detail.html?client=' + encodeURIComponent(clientData.name);
            }, 1500);
        } else {
    ...
    async deleteClient() {
        try {
            ...
            if (result.winCCUaDevice.delete) {
                ...
                this.showError('Failed to delete WinCC Unified client');
            }
        } catch (e) {
            console.error('Delete error', e);
            this.showError('Failed to delete WinCC Unified client: ' + e.message);
        }
    }
