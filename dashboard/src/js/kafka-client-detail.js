...
class KafkaClientDetailManager {
    ...
    showNewClientForm() {
        ...
        // Inbound Defaults
        document.getElementById('client-inbound-enabled').checked = true;
        document.getElementById('client-group-id').value = 'monstermq-subscriber';
        ...
        toggleInboundFields(true);

        // Outbound Defaults
        document.getElementById('client-outbound-enabled').checked = false;
        document.getElementById('client-outbound-topic-filters').value = '';
        document.getElementById('client-outbound-kafka-topic').value = '';
        document.getElementById('client-outbound-topic-key-regex').value = '';
        document.getElementById('client-outbound-topic-key-replacement').value = '';
        document.getElementById('client-extra-producer').value = '';
        toggleOutboundFields(false);
            ...
            const query = `
                query GetKafkaClients($name: String!) {
                    kafkaClients(name: $name) {
                        ...
                        config { 
                            bootstrapServers groupId payloadFormat destinationTopicPrefix topicKeyRegex topicKeyReplacement extraConsumerConfig reconnectDelayMs 
                            inboundEnabled outboundEnabled outboundTopicFilters outboundKafkaTopic outboundTopicKeyRegex outboundTopicKeyReplacement extraProducerConfig 
                        }
                    }
                }
            `;
    ...
    renderClientInfo() {
        ...
        // Inbound Config
        const inboundEnabled = cfg.inboundEnabled !== false;
        document.getElementById('client-inbound-enabled').checked = inboundEnabled;
        document.getElementById('client-group-id').value = cfg.groupId || 'monstermq-subscriber';
        ...
        toggleInboundFields(inboundEnabled);

        // Outbound Config
        const outboundEnabled = cfg.outboundEnabled || false;
        document.getElementById('client-outbound-enabled').checked = outboundEnabled;
        document.getElementById('client-outbound-topic-filters').value = cfg.outboundTopicFilters ? cfg.outboundTopicFilters.join(', ') : '';
        document.getElementById('client-outbound-kafka-topic').value = cfg.outboundKafkaTopic || '';
        document.getElementById('client-outbound-topic-key-regex').value = cfg.outboundTopicKeyRegex || '';
        document.getElementById('client-outbound-topic-key-replacement').value = cfg.outboundTopicKeyReplacement || '';
        document.getElementById('client-extra-producer').value = cfg.extraProducerConfig ? JSON.stringify(cfg.extraProducerConfig, null, 2) : '';
        toggleOutboundFields(outboundEnabled);
    ...
    collectFormData() {
        const inboundEnabled = document.getElementById('client-inbound-enabled').checked;
        ...
        const outboundEnabled = document.getElementById('client-outbound-enabled').checked;
        let extraProducer = null;
        if (outboundEnabled) {
            let extraProducerText = document.getElementById('client-extra-producer').value.trim();
            if (extraProducerText.length > 0) {
                try { extraProducer = JSON.parse(extraProducerText); } catch (e) { return { error: 'Invalid JSON in Extra Producer Config: ' + e.message }; }
                if (extraProducer === null || Array.isArray(extraProducer) || typeof extraProducer !== 'object') {
                    return { error: 'Extra Producer Config must be a JSON object' };
                }
            }
        }

        const outboundTopicFiltersRaw = document.getElementById('client-outbound-topic-filters').value.trim();
        const outboundTopicFilters = outboundTopicFiltersRaw.split(',').map(s => s.trim()).filter(s => s.length > 0);

        if (outboundEnabled && outboundTopicFilters.length === 0) {
            return { error: 'Local MQTT Topic Filters must not be empty when Outbound Bridging is enabled' };
        }
        ...
        return {
            ...
            config: {
                ...
                groupId: inboundEnabled ? document.getElementById('client-group-id').value.trim() : 'monstermq-subscriber',
                destinationTopicPrefix: inboundEnabled ? (function(){ const v=document.getElementById('client-destination-prefix').value.trim(); return v.length>0? v : null; })() : null,
                topicKeyRegex: inboundEnabled ? (function(){ const v=document.getElementById('client-topic-key-regex').value.trim(); return v.length>0? v : null; })() : null,
                topicKeyReplacement: inboundEnabled ? (function(){ const v=document.getElementById('client-topic-key-replacement').value.trim(); return v.length>0? v : null; })() : null,
                extraConsumerConfig: inboundEnabled ? extraConfig : null,

                // Outbound settings
                outboundEnabled: outboundEnabled,
                outboundTopicFilters: outboundTopicFilters,
                outboundKafkaTopic: outboundEnabled ? (function(){ const v=document.getElementById('client-outbound-kafka-topic').value.trim(); return v.length>0? v : null; })() : null,
                outboundTopicKeyRegex: outboundEnabled ? (function(){ const v=document.getElementById('client-outbound-topic-key-regex').value.trim(); return v.length>0? v : null; })() : null,
                outboundTopicKeyReplacement: outboundEnabled ? (function(){ const v=document.getElementById('client-outbound-topic-key-replacement').value.trim(); return v.length>0? v : null; })() : null,
                extraProducerConfig: outboundEnabled ? extraProducer : null
            }
        };
    }
    ...
    async showDeleteModal() {
        ...
        if (await ui.confirmDelete(name, { title: 'Delete Kafka client' })) {
            this.deleteClient();
        }
    }
    goBack() { window.spaLocation.href = '/pages/kafka-clients.html'; }
...
function toggleOutboundFields(checked) {
    const div = document.getElementById('outbound-fields');
    if (div) div.style.display = checked ? 'block' : 'none';
}
function toggleInboundFields(checked) {
    const div = document.getElementById('inbound-fields');
    if (div) div.style.display = checked ? 'block' : 'none';
    const groupId = document.getElementById('client-group-id');
    if (groupId) groupId.required = checked;
}
