class SessionManager {
        ...
        tableBody.innerHTML = sessionsToRender.map(session => {
            ...
            return `
                <tr>
                    ...
                    <td>${session.connected ? `${metrics.inFlightMessagesSnd !== undefined && metrics.inFlightMessagesSnd !== null ? metrics.inFlightMessagesSnd : 0} / ${metrics.inFlightMessagesRcv !== undefined && metrics.inFlightMessagesRcv !== null ? metrics.inFlightMessagesRcv : 0}` : 'N/A'}</td>
                    <td>
    ...
    async removeSessions() {
        ...
        try {
            ...
            const response = await fetch(await window.graphqlClient.resolveEndpoint(), {
                method: 'POST',
                headers: headers,
                body: JSON.stringify({
                    query: mutation,
    ...
    getProtocolBadge(protocolVersion, clientId) {
        if (clientId && clientId.startsWith('nats-')) {
            return '<span class="protocol-badge protocol-nats" style="background: #3B82F6; color: white;">NATS</span>';
        }
        if (clientId && clientId.startsWith('mcp-')) {
            return '<span class="protocol-badge protocol-mcp" style="background: #EC4899; color: white;">MCP</span>';
        }
        if (clientId && clientId.startsWith('redis-')) {
            return '<span class="protocol-badge protocol-redis" style="background: #EF4444; color: white;">REDIS</span>';
        }
